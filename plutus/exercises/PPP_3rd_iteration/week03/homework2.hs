...
mkValidator _beneficiary _dat () _ctx =
  ...
  where
    ...
    signedByBeneficiary = txSignedBy info $ unPaymentPubKeyHash _beneficiary

    deadlineReached :: Bool
    deadlineReached = contains (from _dat) $ txInfoValidRange info
...
typedValidator _beneficiary =
  Scripts.mkTypedValidator @Vesting
    ($$(PlutusTx.compile [||mkValidator||]) `PlutusTx.applyCode` PlutusTx.liftCode _beneficiary)
