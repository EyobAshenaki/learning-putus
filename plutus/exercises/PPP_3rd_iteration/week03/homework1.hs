...
mkValidator dat () ctx =
  ( traceIfFalse "Beneficiary 1's signature missing and/or deadline reached" signedByBeneficiary1
      && traceIfFalse "Deadline has not passed yet" beforeDeadline
  )
    || ( traceIfFalse "Beneficiary 2's signature missing and/or deadline not reached" signedByBeneficiary2
           && traceIfFalse "Deadline has passed already" afterDeadline
       )
  where
    info :: TxInfo
