...
mkMisteryValidator dat () ctx =
    (traceIfFalse "Benificiary1 did not sign "  signedByBeneficiary1 &&
    traceIfFalse "Benificiary1 too late"  checkCondition1 ) ||
   ( traceIfFalse "Benificiary2 did not sign"  signedByBeneficiary2 &&
    traceIfFalse "Benificiary2 is too early" checkCondition2)
    where
        txInfo :: TxInfo
        ...
        -- Checks if the transaction is signed by the beneficiary1 and if the deadline has not passed
        checkCondition1 :: Bool
        checkCondition1 = contains (to (deadline dat)) txValidRange
        signedByBeneficiary1 :: Bool
        signedByBeneficiary1 = txSignedBy txInfo (beneficiary1 dat)

        signedByBeneficiary2 :: Bool
        signedByBeneficiary2 = txSignedBy txInfo (beneficiary2 dat)

        -- Checks if the transaction is signed by the beneficiary2 and if the deadline has passed
        checkCondition2 :: Bool
        checkCondition2 =  contains (from (1 + deadline dat)) txValidRange
